...
output_ratings <- tibble::tibble(
  player = as.factor(c("Duke", "Miami", "UNC", "UVA", "VT")),
...
input_predictions <- tibble::tibble(
  game = c(11,11,12,12),
  player = as.factor(c("VT", "Duke", "Miami", "VT")),
  score = NA
)

output_predictions <- tibble::tibble(
  .pred_class = as.factor(c("winner", "loser", "winner", "loser")),
  # NOTE: The book has this first game as 48-5 but R is rounding down for some reason
  .pred = c(47, 5, 25, 24)
)
...
test_that("massey ratings works", {
  ...
  expect_equal(mod_XY, output_ratings)

  mod_formula <- massey2(score ~ ., ncaa2005) %>%
    purrr::pluck(2) %>%
    dplyr::mutate(dplyr::across(where(is.numeric), round, 1))

  expect_equal(mod_formula, output_ratings)
  ...
  expect_equal(mod_recipe, output_ratings)
})
...
test_that("Input with `player` as factor remains in massey ratings", {
  ...
  expect_equal(output_1, output_ref_1[inds, ])
...
test_that("Unknown players ('NA') are not given rankings", {
  ...
  expect_equal(output_2_x, output_2_y)
})
...
test_that("rate_massey works with numeric `player`", {
  ...
  expect_message(massey2(score ~ ., input),
                 "player datatype will be converted to a factor")
})
...
test_that("rate_massey handles players absent in `cr_data`", {
  ...
  input <-
    ...
    dplyr::mutate(player = forcats::as_factor(player),
                  player = forcats::fct_expand(player, "extra"))

  # TODO: Once the message is more explicit as to what players have a level but not an appearance change this test
  expect_message(massey2(score ~ ., input),
    "Dropping levels in the player factor that do not appear in games"
  )
})
...
test_that("rank_massey works", {
  ...
  expect_equal(output_1, output_rankings)
...
# predict ----------------------------------------------------------------------
test_that("predictions using the massey method work", {
  predictors <-
    ncaa2005 %>%
    dplyr::select(game, player)
  outcome <-
  ...
  pred_XY_class <- predict(mod_XY, input_predictions, type = "class")

  expect_equal(pred_XY_class$.pred_class, output_predictions$.pred_class)

  pred_XY_numeric <- predict(mod_XY, input_predictions, type = "numeric") %>%
    dplyr::mutate(dplyr::across(where(is.numeric), round, 0))

  expect_equal(pred_XY_numeric$.pred, output_predictions$.pred)
})
